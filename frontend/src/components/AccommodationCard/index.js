...
const AccommodationCard = ({ data }) => {
  const {location, rating, name, price, images} = data ? data : {};

  const navigate = useNavigate();

  return (
    <>
      <article className="rounded-xl bg-white p-3 shadow-lg hover:shadow-xl" onClick={() => navigate(`${data.destinationId}`)}>
        <a href="#">
          <div className="relative flex items-end overflow-hidden rounded-xl">
            <img src={images?.[0]} alt="Hotel Photo" />
          ...
          <div className="mt-1 p-2">
            ...
            <p className="text-slate-400 mt-1 text-sm">{location ? location : 'N/A'}</p>
